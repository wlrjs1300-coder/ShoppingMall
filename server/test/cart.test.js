...
test("장바구니를 문자열로 저장하고 다시 안전하게 불러온다", () => {
  ...
  assert.deepEqual(cart.parseCart(stored), [{ ...product, quantity: 2 }]);
...
test("선택 상품의 수량과 합계만 계산한다", () => {
  ...
  assert.deepEqual(summary, { itemCount: 2, selectedItemCount: 1, selectedQuantity: 2, selectedPrice: 24000 });
});
...
test("서버 상품 기준으로 가격을 갱신하고 판매중지 상품을 제거한다", () => {
  ...
  assert.deepEqual(result.cart, [{ id: product.id, name: "새 인절미", price: 13000, quantity: 2, category: "인절미", imageUrl: "new.jpg" }]);
});
