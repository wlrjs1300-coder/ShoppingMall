...
test("정상적인 값으로 회원가입하면 201과 사용자 정보를 반환한다", async () => {
  const res = await request(app).post("/api/users/signup").send(validSignupPayload());

  assert.equal(res.status, 201);
  assert.equal(res.body.user.email, "signup.test@example.com");
  assert.equal(res.body.user.phone, "01012345678");
  assert.equal(res.body.user.password, undefined);
  assert.equal(res.body.user.password_hash, undefined);
});

test("이메일은 소문자로 정규화되어 저장된다", async () => {
  await request(app).post("/api/users/signup").send(validSignupPayload({ email: "Lower.Case@Example.com" }));
  const row = db.prepare("SELECT email FROM user_accounts WHERE email = ?").get("lower.case@example.com");
  assert.ok(row);
});

test("휴대폰 번호는 숫자만 저장된다", async () => {
  await request(app).post("/api/users/signup").send(validSignupPayload({ email: "phone.norm@example.com", phone: "010-9999-8888" }));
...
test("비밀번호는 평문으로 저장되지 않는다", async () => {
  const payload = validSignupPayload({ email: "hash.check@example.com", password: "plainpassword1" });
...
test("회원가입과 동시에 기본 배송지가 함께 생성된다", async () => {
  const res = await request(app).post("/api/users/signup").send(validSignupPayload({ email: "with.address@example.com" }));
...
test("회원가입 성공 시 고객 쿠키가 발급된다", async () => {
  const res = await request(app).post("/api/users/signup").send(validSignupPayload({ email: "cookie.check@example.com" }));
...
test("72바이트 이내의 멀티바이트(한글) 비밀번호는 정상 가입된다", async () => {
  ...
  const res = await request(app).post("/api/users/signup").send(validSignupPayload({ email: "byte.ok@example.com", password: koreanPassword }));
  assert.equal(res.status, 201);
});
...
test("중복 이메일로 가입하면 409가 반환된다", async () => {
  await request(app).post("/api/users/signup").send(validSignupPayload({ email: "dup.signup@example.com" }));
...
test("가입한 계정으로 정상 로그인한다", async () => {
  await request(app).post("/api/users/signup").send(validSignupPayload({ email: "login.ok@example.com", password: "loginpass1" }));
...
test("이메일 대소문자가 달라도 로그인된다", async () => {
  await request(app).post("/api/users/signup").send(validSignupPayload({ email: "case.login@example.com", password: "loginpass1" }));
...
test("잘못된 비밀번호는 401을 반환한다", async () => {
  await request(app).post("/api/users/signup").send(validSignupPayload({ email: "wrong.pw@example.com", password: "loginpass1" }));
...
test("이메일 미존재와 비밀번호 오류의 응답 메시지가 동일하다", async () => {
  await request(app).post("/api/users/signup").send(validSignupPayload({ email: "message.compare@example.com", password: "loginpass1" }));
...
test("탈퇴(withdrawn) 상태 회원은 로그인이 거부된다", async () => {
  await request(app).post("/api/users/signup").send(validSignupPayload({ email: "withdrawn@example.com", password: "loginpass1" }));
...
test("로그인 성공 시 고객 쿠키가 발급된다", async () => {
  await request(app).post("/api/users/signup").send(validSignupPayload({ email: "login.cookie@example.com", password: "loginpass1" }));
...
test("고객 쿠키로 /api/users/me 조회에 성공한다", async () => {
  ...
  await agent.post("/api/users/signup").send(validSignupPayload({ email: "me.check@example.com" }));
...
test("로그아웃 이후에는 /api/users/me 접근이 실패한다", async () => {
  const agent = request.agent(app);
  await agent.post("/api/users/signup").send(validSignupPayload({ email: "logout.flow@example.com" }));
