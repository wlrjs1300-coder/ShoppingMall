...
test("관리자 결제 재조정 UI는 RECONCILE_REQUIRED에만 버튼을 표시한다", () => {
  assert.match(orders, /order\.paymentInternalStatus === "RECONCILE_REQUIRED"/);
...
test("주문 응답의 결제 요약을 사용하고 주문별 결제 조회를 실행하지 않는다", () => {
  assert.match(orders, /order\.paymentInternalStatus/);
  assert.match(orders, /order\.paymentLastError/);
  assert.match(orders, /order\.paymentUpdatedAt/);
  assert.doesNotMatch(orders, /hydrateAdminPaymentStates|adminPaymentStateLoads|adminPaymentStates/);
  assert.doesNotMatch(orders, /apiFetch\(`\/payments\/\$\{encodeURIComponent\(order\.id\)\}`\)/);
});
...
test("재조정 예외는 성공 여부에 따라 안내를 구분하고 항상 UI 잠금을 해제한다", () => {
  ...
  assert.match(reconciliation, /if \(!refreshedOrder\) throw/);
  assert.doesNotMatch(reconciliation, /refreshedPayment|\/payments\/\$\{encodeURIComponent\(orderId\)\}`\)(?!\/reconcile)/);
